...
def get_signature_parameters(
    ...
    params = {}
    cache: Dict[str, Any] = {}
    eval_annotation = nexon.utils.evaluate_annotation
        ...
        try:
            ...
            if cog is not None:
                ...
                if local_check is not None:
                    ret = await nexon.utils.maybe_coroutine(local_check, ctx)
                    if not ret:
                        return False
            ...
            return await nexon.utils.async_all(predicate(ctx) for predicate in predicates)  # type: ignore
        finally:
...
def has_role(item: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx: Context) -> bool:
        ...
        if isinstance(item, int):
            role = nexon.utils.get(ctx.author.roles, id=item)  # type: ignore
        else:
            role = nexon.utils.get(ctx.author.roles, name=item)  # type: ignore
        if role is None:
...
def has_any_role(*items: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx) -> bool:
        ...
        # ctx.guild is None doesn't narrow ctx.author to Member
        getter = functools.partial(nexon.utils.get, ctx.author.roles)
        if any(
...
def bot_has_any_role(*items: int) -> Callable[[T], T]:
    ...
    def predicate(ctx) -> bool:
        ...
        me = ctx.me
        getter = functools.partial(nexon.utils.get, me.roles)
        if any(
            getter(id=item) is not None if isinstance(item, int) else getter(name=item) is not None
            for item in items
        ):
...
def dynamic_cooldown(
    ...
    accepts a single parameter of type :class:`.nexon.Message` and must
    ...
    Parameters
    ----------
    cooldown: Callable[[:class:`.nexon.Message`], Optional[:class:`.Cooldown`]]
