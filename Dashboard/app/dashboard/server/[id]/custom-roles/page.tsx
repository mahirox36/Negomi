...
export default function CustomRole() {
  ...
  // State setup
  const [settings, setSettings] = useState<CustomRoleSettings>({...defaultSettings});
  const [originalSettings, setOriginalSettings] = useState<CustomRoleSettings>({...defaultSettings});
      ...
      if (settingsRes.data) {
        ...
        if (data) {
          settingsData = {
            ...
            not_allowed: Array.isArray(data.not_allowed) ? data.not_allowed : defaultSettings.not_allowed,
            allowed_roles: Array.isArray(data.allowed_roles) ? data.allowed_roles : defaultSettings.allowed_roles,
            max_roles_per_user: data.max_roles_per_user || defaultSettings.max_roles_per_user,
            can_hoist: data.can_hoist !== undefined ? data.can_hoist : defaultSettings.can_hoist,
            can_mention: data.can_mention !== undefined ? data.can_mention : defaultSettings.can_mention,
            color_restriction: data.color_restriction || defaultSettings.color_restriction,
            max_members_per_role: data.max_members_per_role || defaultSettings.max_members_per_role,
            require_confirmation: data.require_confirmation !== undefined ? data.require_confirmation : defaultSettings.require_confirmation,
            parent_role_id: data.parent_role_id || defaultSettings.parent_role_id,
          };
        }
      }
  ...
  const toggleFeature = async () => {
    ...
    try {
      ...
      await axios.post(`/api/v1/guilds/${serverId}/features/roles/${endpoint}`, {}, {
        withCredentials: true
      });
  ...
  const handleSettingChange = <K extends keyof CustomRoleSettings>(key: K, value: CustomRoleSettings[K]) => {
  ...
  const addProhibitedWord = () => {
    ...
    handleSettingChange('not_allowed', [...settings.not_allowed, prohibitedWord.toLowerCase()]);
    setProhibitedWord("");
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-lg text-white/70 mt-1">
              Configure how server members can create and manage their custom roles
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="p-6 space-y-8">
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <OptionCard
                ...
                onClick={() => handleSettingChange('creation_mode', 'specific_roles')}
              />
            </div>
            ...
            {settings.creation_mode === 'specific_roles' && (
              <div className="mt-4 p-4 rounded-lg bg-white/5 border border-white/10">
                <h3 className="text-sm font-medium text-white mb-3">Select Allowed Roles</h3>
                <DiscordSelect
                  type="role"
                  guildId={serverId}
                  value={settings.allowed_roles}
                  onChange={(value) => handleSettingChange("allowed_roles", value as string[])}
          ...
          >
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-medium text-white mb-2">Parent Role</h3>
                ...
                <DiscordSelect
                  ...
                  onChange={(value) => handleSettingChange("parent_role_id", value as string)}
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="text-xs text-amber-100/70 space-y-1.5 list-disc ml-4">
                        <li>Setting this too high in the role hierarchy could be dangerous</li>
                        <li>Custom roles will inherit permissions from roles above them</li>
                        <li>Members could gain unintended permissions if the parent role is too high</li>
                        <li>Recommended: Create a dedicated "Custom Roles" role with minimal permissions</li>
                      </ul>
                    </div>
                  </div>
                </div>
            ...
            >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleSettingChange('max_roles_per_user', parseInt(e.target.value))}
              ...
              <div className="pt-4">
                ...
                <input
                  ...
                  onChange={(e) => handleSettingChange('max_members_per_role', parseInt(e.target.value))}
                  className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer accent-purple-500"
                />
                <div className="flex justify-between text-xs text-white/50 mt-1">
                  <span>1</span>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div
                    ...
                    onClick={() => handleSettingChange('color_restriction', 'all')}
                  >
                    ...
                    <span className="text-xs font-medium text-white">Any Color</span>
                  </div>
                  ...
                  <div
                    ...
                    onClick={() => handleSettingChange('color_restriction', 'preset')}
                  >
                    <div className="flex gap-1 mb-2">
                      <span className="w-3 h-3 rounded-full bg-purple-500"></span>
                      <span className="w-3 h-3 rounded-full bg-blue-500"></span>
                    </div>
                    <span className="text-xs font-medium text-white">Preset Only</span>
                  </div>
                  ...
                  <div
                    ...
                    onClick={() => handleSettingChange('color_restriction', 'none')}
                  >
                    <div className="flex gap-1 mb-2">
                      <span className="w-3 h-3 rounded-full bg-gray-500"></span>
                    </div>
                    <span className="text-xs font-medium text-white">No Colors</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-white">Allow Hoisting</h4>
                    <p className="text-xs text-white/50">Let members display their roles separately in the member list</p>
                  </div>
                  <ToggleSwitch 
                    checked={settings.can_hoist} 
                    onChange={(value: boolean) => handleSettingChange('can_hoist', value)}
                    theme="purple"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-white">Allow Mentionable Roles</h4>
                    <p className="text-xs text-white/50">Let members make their roles mentionable by anyone</p>
                  </div>
                  <ToggleSwitch 
                    checked={settings.can_mention} 
                    onChange={(value: boolean) => handleSettingChange('can_mention', value)}
                    theme="purple"
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-white">Require Confirmation</h4>
                    <p className="text-xs text-white/50">Users must confirm before being added to a custom role</p>
                  </div>
                  <ToggleSwitch 
                    checked={settings.require_confirmation} 
                    onChange={(value: boolean) => handleSettingChange('require_confirmation', value)}
                    theme="purple"
                  />
                </div>
              </div>
